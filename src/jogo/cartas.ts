...
export interface ResultadoDaBusca {
  cartas: Carta[]
  /** Quantas casaram no total, mesmo além do limite mostrado. */
  total: number
}
 ...
 * de "Nami & Zeus".
 */
export function buscarCartas(deck: Carta[], texto: string, limite = 12): ResultadoDaBusca {
  const termos = texto.split(/\s+/).map(chaveDeBusca).filter((termo) => termo !== '')
  if (termos.length === 0) return { cartas: [], total: 0 }
  ...
  return { cartas: achados.slice(0, limite).map((achado) => achado.item.carta), total: achados.length }
}
